...
function EndpointResource(resourceName) {
    ...
    this.objectWriter = new ObjectWriter().setName(resourceName);
...
function setParent(p) {
    ...
    if (p.objectWriter) {
        if (!this.name)
            throw new Error('FunctionWriter needs to have a name before being appended');
