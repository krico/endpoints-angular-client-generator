...
function generate(what, realOut, callback) {
    var resource;
    if (what.constructor == RestDescription) {
        resource = what;
    } else {
        resource = new RestDescription(what);
    }
    resource.validate();
    ...
    resource.methodNames().forEach(function (value) {
    ...
    providerWriters.push(new Statement('' + Constants.PROVIDER + '.config={apiRoot:\'' + resource.baseUrl() + '\'};'));
